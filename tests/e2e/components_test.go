...
func (tc *ComponentTestCtx) ValidateDeploymentDeletionRecovery(t *testing.T) {
	...
	for _, deployment := range deployments {
		t.Run("deployment_"+deployment.GetName(), func(t *testing.T) {
			...
			deploymentName := resources.NamespacedNameFromObject(&deployment)
...
func (tc *ComponentTestCtx) ValidateConfigMapDeletionRecovery(t *testing.T) {
	...
	for _, configMap := range configMaps {
		t.Run("configMap_"+configMap.GetName(), func(t *testing.T) {
			...
			configMapName := resources.NamespacedNameFromObject(&configMap)

			// Delete the ConfigMap
			tc.DeleteResource(
				WithMinimalObject(gvk.ConfigMap, configMapName),
			)
...
func (tc *ComponentTestCtx) ValidateServiceDeletionRecovery(t *testing.T) {
	...
	for _, service := range services {
		t.Run("service_"+service.GetName(), func(t *testing.T) {
			...
			serviceName := resources.NamespacedNameFromObject(&service)

			// Delete the Service
			tc.DeleteResource(
				WithMinimalObject(gvk.Service, serviceName),
			)
...
func (tc *ComponentTestCtx) ValidateRouteDeletionRecovery(t *testing.T) {
	...
	for _, route := range routes {
		t.Run("route_"+route.GetName(), func(t *testing.T) {
			...
			routeName := resources.NamespacedNameFromObject(&route)

			// Delete the Route
			tc.DeleteResource(
				WithMinimalObject(gvk.Route, routeName),
			)
...
func (tc *ComponentTestCtx) ValidateServiceAccountDeletionRecovery(t *testing.T) {
	...
	for _, serviceAccount := range serviceAccounts {
		t.Run("serviceAccount_"+serviceAccount.GetName(), func(t *testing.T) {
			...
			serviceAccountName := resources.NamespacedNameFromObject(&serviceAccount)

			// Delete the ServiceAccount
			tc.DeleteResource(
				WithMinimalObject(gvk.ServiceAccount, serviceAccountName),
			)
...
func (tc *ComponentTestCtx) ValidateRBACDeletionRecovery(t *testing.T) {
	...
	for _, clusterRoleBinding := range clusterRoleBindings {
		t.Run("clusterRoleBinding_"+clusterRoleBinding.GetName(), func(t *testing.T) {
			...
			clusterRoleBindingName := resources.NamespacedNameFromObject(&clusterRoleBinding)

			// Delete the ClusterRoleBinding
			tc.DeleteResource(
				WithMinimalObject(gvk.ClusterRoleBinding, clusterRoleBindingName),
			)
	...
	for _, clusterRole := range clusterRoles {
		t.Run("clusterRole_"+clusterRole.GetName(), func(t *testing.T) {
			...
			clusterRoleName := resources.NamespacedNameFromObject(&clusterRole)

			// Delete the ClusterRole
			tc.DeleteResource(
				WithMinimalObject(gvk.ClusterRole, clusterRoleName),
			)
	...
	for _, roleBinding := range roleBindings {
		t.Run("roleBinding_"+roleBinding.GetName(), func(t *testing.T) {
			...
			roleBindingName := resources.NamespacedNameFromObject(&roleBinding)

			// Delete the RoleBinding
			tc.DeleteResource(
				WithMinimalObject(gvk.RoleBinding, roleBindingName),
			)
	...
	for _, role := range roles {
		t.Run("role_"+role.GetName(), func(t *testing.T) {
			...
			roleName := resources.NamespacedNameFromObject(&role)

			// Delete the Role
			tc.DeleteResource(
				WithMinimalObject(gvk.Role, roleName),
			)
