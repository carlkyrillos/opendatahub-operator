...
func (w *NotebookWebhook) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	case admissionv1.Create, admissionv1.Update:
		...
		injectionPerformed, obj, err := w.performConnectionInjection(ctx, req, secretRefs)
		if err != nil {
...
func (w *NotebookWebhook) performConnectionInjection(ctx context.Context, req admission.Request, secretRefs []corev1.SecretReference) (bool, *unstructured.Unstructured, error) {
	log := logf.FromContext(ctx)
	...
	// Get existing envFrom array
	existingEnvFrom, found, err := unstructured.NestedSlice(obj.Object, "spec", "template", "spec", "containers", "0", "envFrom")
	if err != nil {
		log.Error(err, "failed to get envFrom array")
		return false, nil, fmt.Errorf("failed to get envFrom array: %w", err)
	}
	if !found {
		existingEnvFrom = make([]interface{}, 0)
	}

	// Keep only non-secretRef entries (like configMapRef)
	var preservedEntries []interface{}
	for _, entry := range existingEnvFrom {
		// If the entry is a map and it has "secretRef" key, skip it; otherwise keep it
	...
	// Set the updated envFrom array
	if err := unstructured.SetNestedSlice(obj.Object, newEnvFrom, "spec", "template", "spec", "containers", "0", "envFrom"); err != nil {
		log.Error(err, "failed to set envFrom array")
		return false, nil, fmt.Errorf("failed to set envFrom array: %w", err)
	}
