...
def load_component_mapping(path: Path | None = None) -> dict[str, ComponentMapping]:
    ...
    if not mapping_path.exists():
        log.warning("Component mapping file not found: %s", mapping_path)
        return {}
...
def parse_classification(testcase: ET.Element) -> FailureClassification | None:
...
class JiraClient:
    ...
    def search_open_blockers(self, jira_component: str) -> list[dict]:
        ...
        try:
            resp = self.session.get(
                ...
                params={"jql": jql, "maxResults": 5, "fields": "key,summary,status"},
                timeout=30,
            )
    ...
    if not result.component_failures:
        ...
        for cf in result.component_failures:
            ...
            if mapping and mapping.jira_component:
                ...
                print(f"  Jira component: NOT MAPPED")
